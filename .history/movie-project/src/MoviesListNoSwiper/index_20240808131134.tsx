...
const MoviesListNoSwiper = ({ url }: { url: string }) => {
  ...
  return (
    <>
      {movies && movies.results.length > 0 && movies.results.map((movie) => (
        <div className="movie-box" key={movie.id}> {/* Thêm key vào đây */}
          <Link to={`/Movie/${movie.id}`}>
            <img src={`https://www.themoviedb.org/t/p/w220_and_h330_face${movie.poster_path}`} alt={movie.title} className="movie-box-img" />

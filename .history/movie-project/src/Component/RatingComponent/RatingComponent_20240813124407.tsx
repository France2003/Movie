...
const RatingComponent = ({ movieId }: RatingComponentProps) => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            setSubmittedRatings([...submittedRatings, rating]); // Cập nhật danh sách đánh giá đã gửi
        ...
        <div className="p-4 bg-[#1e1e2a] rounded-lg shadow-md mt-4">
            {submittedRatings.length > 0 && (
                <div className="mt-4 p-5 bg-gray-800 rounded-md text-white leading-8">
                    <h4 className="text-lg font-semibold">Your Submitted Reviews:</h4>
                    <ul className="list-disc pl-5">
                        {submittedRatings.map((rating, index) => (
                            <li key={index} className="flex gap-2 items-center">
                                <IoEllipseSharp className="text-yellow-400" /> {rating}
                            </li>
                        ))}
                    </ul>
                </div>
            ...
            <button
                ...
                className="bg-blue-600 text-white px-4 py-2 rounded-md mt-2 transition-transform duration-200 hover:scale-105 active:scale-95"
            >
                Submit Rating
            </button>

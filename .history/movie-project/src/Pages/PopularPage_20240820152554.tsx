...
const PopularPage = () => {
  ...
  const handlePageChange = (page: number) => {
    if (page > 0 && page <= totalPages) {
      setCurrentPage(page);
      setIsLoading(true); // Set loading state when the page changes
    }
  };
  ...
  }, [isLoading]); // Rerun the effect when loading state changes
  ...
  return (
    <>
      ...
      <section className="popular container popular-route" id="popular">
        ...
        <div className={`popular-content swiper flex gap-5 flex-wrap justify-between animate-from-right ${isLoading ? '' : 'loaded'}`}>
          <MoviesListNoSwiper
            url={`/popular`}
            page={currentPage}
            setTotalPages={(total) => {

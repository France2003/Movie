        ...
        <>
          ...
          {showVideo && fullMovieUrl && (
            <div
              ...
              style={{ animation: showVideo ? "fadeIn 0.3s ease-in-out forwards" : "fadeOut 0.3s ease-in-out forwards" }}
            >
              <div className="bg-gray-900 p-5 rounded-lg shadow-lg">
                <button onClick={handleCloseVideo} className="text-white absolute top-2 right-2">
                  <FiX size={24} />
                </button>
                <iframe
                  width="800"
            ...
            <div className="movie-grid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {similarMovies && similarMovies.map((movie) => (
                <li key={movie.id} className="bg-[#1e1e2a] rounded-lg shadow-md flex items-center p-4 hover:shadow-lg transition-shadow duration-300">
                  <Link to={`/movie/${movie.id}`} className="flex flex-col items-center group hover:bg-gray-800 rounded-lg overflow-hidden transition-transform duration-300">
                    <img
                      src={`https://www.themoviedb.org/t/p/w220_and_h330_face${movie.poster_path}`}
                      alt={movie.original_title}
                      className="w-full h-auto object-cover rounded-t-lg transition-transform duration-300 group-hover:scale-105"
                      style={{ border: '1px solid white' }} // Đặt đường viền trắng
                    />
                    <div className="p-2 w-full text-center">
                      <h2 className="text-lg font-semibold text-white truncate">{movie.original_title}</h2>
                    </div>
                  </Link>

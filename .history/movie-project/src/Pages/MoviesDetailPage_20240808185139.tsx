...
const MoviesDetailPage = () => {
  ...
  useEffect(() => {
    if (movie && movie.videos.results.length > 0) {
      // Tìm video dài hoặc loại khác
      const preferredVideos = movie.videos.results.filter(
        (video) => video.site === "animevietsub"
      );

      const selectedVideo = preferredVideos.length > 0 ? preferredVideos[0] : undefined;
      const url = selectedVideo ? `https://animevietsub.app/embed/${selectedVideo.key}` : undefined;
      setFullMovieUrl(url);
    }
  }, [movie]);

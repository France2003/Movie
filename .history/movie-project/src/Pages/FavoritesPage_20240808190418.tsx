...
const FavoritesPage = () => {
  const [favorites, setFavorites] = useState<{ id: number; addedAt: number }[]>([]);
  const [movies, setMovies] = useState<any[]>([]);
  const [notification, setNotification] = useState<string | null>(null);

  useEffect(() => {
    const storedFavorites = JSON.parse(localStorage.getItem("favorites") || "[]");
    // Sort favorites by the time they were added (newest first)
    const sortedFavorites = storedFavorites.sort((a: { addedAt: number }, b: { addedAt: number }) => b.addedAt - a.addedAt);
  ...
  const addFavorite = (movieId: number) => {
    const addedAt = Date.now(); // current timestamp
    const newFavorites = [...favorites, { id: movieId, addedAt }];

...
const FavoritesPage = () => {
  ...
  useEffect(() => {
    if (favorites.length > 0) {
      const fetchMovies = async () => {
        ...
        // Create a map for quick access to movies by their IDs
        const moviesMap = moviesData.reduce((acc, res) => {
          acc[res.data.id] = res.data;
          return acc;
        }, {} as Record<number, any>);

        // Create a sorted list based on the order of IDs in the `favorites` array
        const sortedMovies = favorites.map((id) => moviesMap[id]);
  ...
  const removeFavorite = async (movieId: number) => {
    ...
    // Tùy chọn: Cập nhật danh sách phim nếu cần
    ...
    <div className="p-6 bg-[#1e1e2a] mt-[70px] m-auto w-[1200px] animate__animated animate__fadeIn">
      ...
      {loading ? (
        <div className="loading-spinner m-auto">
          <div className="spinner mb-[200px]"></div>
        </div>
      ) : movies.length > 0 ? (
